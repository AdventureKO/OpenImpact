...
export async function calculateTransparencyScore(projectId) {
  try {
    const incoming = await loadIncomingDonations();
    const causeDonations = (incoming || []).filter(
    ...
    for (const don of causeDonations) {
      const assigns = await assignmentsForDonation(
    ...
    return {
      assigned,
      total,
      percentageAssigned: Math.round((assigned / total) * 100),
    };
  } catch (e) {
