...
export default function ProfileDropdown({ onLogout }) {
  ...
  const dumpPublicStorage = async () => {
    try {
      ...
      console.log(
        "DEBUG dumpPublicStorage: publicFundraisers count=",
        (pub || []).length,
      );
  ...
  const resetPublicprojects = async () => {
    try {
      if (!auth || !auth.user || !auth.user.isAdmin) {
        return Alert.alert(
          "Permission",
          "Only admin can reset public fundraisers",
        );
      }
      ...
      Alert.alert(
        "Reset",
        `publicFundraisers replaced with bundled list (${(normalized || []).length} items)`,
      );
    } catch (e) {
  ...
  const exportPublicToJson = async () => {
    try {
      if (!auth || !auth.user || !auth.user.isAdmin)
        return Alert.alert(
          "Permission",
          "Only admin can export public fundraisers",
        );
      ...
      Alert.alert(
        "Export ready",
        "Public fundraisers JSON is available in the modal and console logs. Copy and paste into fundraisers.json in your project.",
      );
    } catch (e) {
  ...
  const exportSavedListsToJson = async () => {
    try {
      if (!auth || !auth.user)
        return Alert.alert(
          "Permission",
          "Please sign in to export your saved lists",
        );
      const raw =
        (await storage.loadForUser(auth.user, "savedLists", [])) || [];
      ...
      Alert.alert(
        "Export ready",
        "Your saved lists JSON is available in the modal and console logs. Copy and paste as needed.",
      );
    } catch (e) {
    ...
    const seed = [
      {
        ...
        method:
          "Collect and distribute non-perishable food items to local shelters.",
        published: false,
      },
      {
        id: "my-2",
        ...
        method:
          "Establish a community garden plot and recruit volunteers for planting and maintenance.",
        published: false,
      },
      {
        id: "my-3",
        ...
        method:
          "Raise funds and collect school supplies to distribute to students in need.",
        published: false,
      },
    ];
        ...
        <View style={[styles.modalWrap, { backgroundColor: bg }]}>
          <View style={styles.modalInner}>
            {avatar ? (
              ...
              <View
                style={[styles.largePlaceholder, { backgroundColor: "#888" }]}
              >
            ...
            {auth && auth.user && auth.user.isAdmin ? (
              <View
                style={{ flexDirection: "column", marginTop: 8, width: "100%" }}
              >
                <TouchableOpacity
                  onPress={dumpPublicStorage}
                  style={[
                    styles.button,
                    styles.adminButton,
                ...
                >
                  <Text style={{ color: "#fff" }}>Dump Public Storage</Text>
                </TouchableOpacity>
                <View
                  style={{
                    flexDirection: "row",
                ...
                >
                  <TouchableOpacity
                    onPress={resetPublicprojects}
                    style={[
                      styles.button,
                      styles.adminButtonHalf,
                      { backgroundColor: "#c0392b" },
                    ]}
                  >
                    <Text style={{ color: "#fff" }}>Reset Public Causes</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={exportPublicToJson}
                    style={[
                      styles.button,
                      styles.adminButtonHalf,
            ...
            <View style={{ marginTop: 8, width: "100%" }}>
              ...
              >
                ...
                >
                  <Text
                    style={{
                      color: themeSelection === "system" ? "#fff" : "#000",
                    }}
                  >
                    System
                  </Text>
                ...
                <TouchableOpacity
                  onPress={async () => {
                    await themePref.setTheme("light");
                ...
                >
                  <Text
                    style={{
                      color: themeSelection === "light" ? "#fff" : "#000",
                    }}
                  >
                    Light
                  </Text>
                ...
                >
                  <Text
                    style={{
                      color: themeSelection === "dark" ? "#fff" : "#000",
                    }}
                  >
                    Dark
                  </Text>
            ...
            {auth && auth.user ? (
              <View style={{ marginTop: 8, width: "100%" }}>
                <TouchableOpacity
                  onPress={exportSavedListsToJson}
                  style={[styles.button, { backgroundColor: "#16a085" }]}
                >
                  <Text style={{ color: "#fff" }}>Export My Saved Lists</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() => {
                    setVisible(false);
                ...
                >
                  <Text style={{ color: "#fff" }}>My Donations</Text>
                </TouchableOpacity>
              </View>
            ) : null}
            ...
            <TouchableOpacity
              ...
              style={styles.close}
            >
              <Text style={{ color: text }}>Close</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
            ...
            <View style={{ flexDirection: "row", marginTop: 8 }}>
              <TouchableOpacity
                onPress={() => {
                  ...
                  Alert.alert(
                    "Copy",
                    "JSON printed to console. Long-press the text above to copy it, or copy from Metro logs.",
                  );
                }}
