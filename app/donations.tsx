  ...
  const renderItem = ({ item }) => (
    <View style={styles.row}>
      <View style={{ flex: 1 }}>
        ...
        <Text style={{ color: text, fontSize: 12, marginTop: 6 }}>Project: {item.projectId || '—'}</Text>
        <Text style={{ color: text, fontSize: 12 }}>Date: {new Date(item.createdAt).toLocaleString()}</Text>
      </View>
      <TouchableOpacity onPress={() => {
        Alert.alert('Donation', `ID: ${item.id}\nAmount: ${item.amount}\nProject: ${item.projectId || '—'}`);
  ...
  return (
    <SafeAreaView style={[{ flex: 1, backgroundColor: bg }]}> 
      <View style={{ padding: 16 }}>
        <Text style={[{ fontSize: 22, fontWeight: '700', marginBottom: 8, color: text }]}>My Donations</Text>
        {donations.length === 0 ? <Text style={{ color: text }}>No donations yet.</Text> : (
          <FlatList data={donations} keyExtractor={d => d.id} renderItem={renderItem} />
        )}
      </View>
    </SafeAreaView>
  );
