...
export default function ProjectsScreen() {
  ...
  return (
    <SafeAreaView style={{flex:1, backgroundColor: bg, padding:16, paddingTop:12, paddingBottom:160}}>
      <Text style={{fontSize:20, fontWeight:'700', marginBottom:8, color: text}}>Browse Causes</Text>
      <Text style={{color: text, marginBottom:12, fontSize: 14}}>Search and discover projects to support.</Text>

      <View style={{marginBottom:16}}>
        <TextInput placeholder="Search causes or organizers" value={query} onChangeText={setQuery} style={{borderWidth:1, borderColor:'#e0e0e0', padding:10, borderRadius:8, backgroundColor: surface || bg, color: text}} placeholderTextColor={secondary || '#999'} />
      </View>

      <Text style={{fontWeight:'700', marginBottom:8, color: text}}>Results ({results.length})</Text>
      <FlatList
        data={results}
        keyExtractor={i=>i.id}
        contentContainerStyle={{ paddingBottom: 140 }}
        renderItem={({item}) => (
          <View style={{padding:12, borderRadius:8, backgroundColor:surface || '#fff', marginBottom:8, borderWidth: 1, borderColor: '#e0e0e0'}}>
            <Text style={{fontWeight:'700', color: text}}>{item.name}</Text>
            <Text style={{color: secondary, marginTop: 4, fontSize: 13}}>{item.organizer || item.writer}</Text>
            <Text style={{fontSize:12, color: secondary, marginTop:6}}>{Math.round(((item.current||0)/(item.goal||1))*100)}% • ${item.current||0} raised</Text>
